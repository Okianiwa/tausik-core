...
поставил 2.0.0, и все шесть серверов из `harness/claude/mcp/` и
`harness/cursor/mcp/` начали падать на старте с `AttributeError` ещё до JSON-RPC
цикла.
...
def test_direct_api_deps_are_capped():
    ...
    assert not uncapped, (
        ...
        "harness/{claude,cursor}/mcp/*/server.py на старте — молча, с одним "
