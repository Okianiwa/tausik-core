...
# No "review": nothing in production ever ran gates on it (`tausik review`
# records L1/L2/L3 runs per SENAR 10.15 and never calls the gate runner), so a
# gate assigned to it was unreachable by construction — which is exactly how
# bandit sat disabled-and-unrunnable. Task gates-registry-split-cli-vs-hook.
VALID_GATE_TRIGGERS = frozenset({"task-done", "verify", "commit"})

# Command-security rules live in `gate_command_security`; re-exported here
# because callers (and the test suite) have imported them from this module
# since before the split.
from gate_command_security import (  # noqa: E402,F401
    ALLOWED_GATE_EXECUTABLES,
    _SHELL_CHAIN_PATTERN,
    _SHELL_INJECTION_PATTERN,
    validate_custom_gate as _validate_custom_gate,
)
...
from default_gates import CATALOG_GATES, DEFAULT_GATES  # noqa: E402


def _build_stack_gate_map() -> dict[str, list[str]]:
    """Build mapping: stack -> list of gates to auto-enable.

    Built from CATALOG_GATES, not DEFAULT_GATES: bootstrap calls
    `auto_enable_gates_for_stacks` for stacks it has just DETECTED, before the
    deploy that would put them in the active registry. Reading the active
    registry here makes that circular — terraform-validate could only be
    auto-enabled for a project that already had the terraform stack deployed.
    """
    result: dict[str, list[str]] = {}
    for gate_name, gate_cfg in CATALOG_GATES.items():
