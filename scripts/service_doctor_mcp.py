...
def discover_servers(project_dir: str) -> list[tuple[str, str]]:
    """Every `<profile>/mcp/*/server.py`, sorted by name.
    ...
    The profile directory is resolved, not assumed to be `.claude`: bootstrap
    deploys `.cursor/mcp`, `.qwen/mcp` and friends, and a hardcoded `.claude`
    made doctor declare every Cursor/Qwen install broken. A health check that
    fails healthy projects trains people to ignore it.
    """
    from ide_utils import resolve_profile

    _ide, ide_rel = resolve_profile(project_dir)
    root = os.path.join(project_dir, ide_rel, "mcp")
...
def check_mcp_servers(
    ...
    if not servers:
        return [("fail", "MCP servers", "no <profile>/mcp/*/server.py at all — re-run bootstrap")]
