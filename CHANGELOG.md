...
- **The `mypy` gate blocked a commit it had checked nothing in.** The command deliberately carries no `{files}` and takes its scope from `[tool.mypy]` (`files = ["scripts"]`, `exclude = ["scripts/hooks/"]`). On the `commit` trigger the gate judges a temp tree into which `checkout-index` places **staged content only**, so a commit touching just `scripts/hooks/` left that config resolving to nothing: mypy exited with the usage error `There are no .py[i] files in directory 'scripts'` and severity=block rejected a commit in which type-checking never ran. Caught on the first commit of `scripts/hooks/` changes. Empty input is not a failure — `gate_command_runner` recognises the message and returns a distinct `_NOTHING_TO_CHECK_SENTINEL`, and `run_gates` reports SKIP with a reason naming the real cause (kept separate from the scoped sentinel, which would otherwise explain it as unmapped test files). Genuine type errors carry a different message and still block — pinned by a pass/fail pair in `test_gates.py`, the negative one verified by mutation.
