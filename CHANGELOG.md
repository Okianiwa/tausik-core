...
- **The window was wiped wherever a 45-second pause happened to fall, and the run was never told the context was filling up (`wrap-up-before-window-wipe`).** On reaching the threshold the watcher went straight to `/checkpoint` → `/clear` → `/start`, so the cleanup could land halfway through a task; meanwhile nothing informed the agent, which kept taking on more work. The Stop hook could already ask for a wind-down, but `exit_guard.decide` gates the soft branch on `task_state == STATE_COMPLETE` — for a task ALREADY closed. Mid-task the agent hears nothing until `hard_threshold` = 75%. A `winding` state now sits between the countdown and the wipe: the watcher types one line asking the run to finish the current task, close it properly and start nothing new, then waits for the run to actually STAND — the Stop hook's flag, not the transcript, since a quiet transcript means the turn ended and not the work. `WIND_TIMEOUT` = 1800s wipes anyway, because a wind-down able to veto the cleanup hands back the very defect the mechanism exists for. Two things had to be handled that only appear once the request is real: the watcher types INTO the chat, so its own request lands as a turn from the human and the «somebody came back» branch would cancel the cleanup that sent it (`echo_of_us`, 5s tolerance); and the silence that armed the countdown was measured BEFORE the request was typed, so judging by it in the same tick wipes the window before the request can be read (the loop defers to the next tick). Found while restructuring: `run_sequence` left the cycle ARMED and the only thing moving it on was that same false cancel firing a tick later against the run's own writing — visible in the log two seconds after both completed cleanups, at 22:43:24 and 19:09:26. Without that accident the sequence would have repeated every tick; `Maintenance.finish` now does it explicitly. `DEFAULT_SOFT_THRESHOLD` moves 30 → 50.
