...
- **The chat watcher hands a stalled run its direction back without waiting for the window to fill (`heartbeat-blocks-context-cleanup`).** The cleanup re-reads the rules as a side effect of `/clear` → `/start`, but it fires on the window, and a run stalls for a different reason — a step ended, nobody started the next one, and the human who would nudge it is away: that is why the run was declared. After 20 minutes of silence across the whole transcript (`ANCHOR_SECONDS`) the watcher delivers the same continuation line the cleanup ends with. It goes through the continuation path — no `Esc`, one attempt — so a half-typed draft is not wiped and an agent mid-turn is not interrupted. Every condition is a way of not talking over somebody: background work in flight, an armed cleanup (which does strictly more), and the same 45 seconds of quiet all stop the nudge. The quiet is the whole transcript's, not the human's turns as in the cancel path: there the agent's own steps are not somebody walking in, here they are the proof the run is alive. Three deliveries in a row that moved no text mean finished, not stalled (`ANCHOR_TRIES`): the watcher logs that and leaves the chat alone for the rest of the run, because typing into an empty queue until morning is worse than stopping and saying so. 7 cases in `test_autoloop_watch.py`.
