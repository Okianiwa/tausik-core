...
- **The context cleanup was cancelled by a screen that ticks, not by a person typing (`cleanup-cancelled-by-live-screen`).** `draft_changed` compared the bottom twelve rows of the chat's console between arming and firing, on the reasoning that a growing draft is the one thing the transcript's mtime cannot see. Those twelve rows also hold the spinner and the status bar. Measured on two live chats through `console_text`, six seconds apart, with no key pressed: row 3 moved (`Zigzagging… (4m 2s → 4m 8s · ↓ 12.0k → 12.5k tokens)`) and row 9 moved (`Usage 21% (resets in 2h 14m → 2h 13m)`). Over a fifteen-second countdown that reads as somebody at the keyboard. The cost in one day of `D:/asynchronus/.tausik/chat-watch.log`: 31 arms, 6 cleanups, 23 cancels logged as «человек печатает» — each exactly 16s after its arm, at 13:29, 16:11, 18:44, 22:31, which is not how a person types. Every cancel buys `CANCEL_QUIET` = 600s of refusal, so the window climbed 30 → 33 → 37.9 → 44.5 → 47.7 → 50.2% uncleaned. The draft is not anywhere on that screen: the layout, identical on both chats, is conversation, spinner, rule, `❯`, rule, status bar — so the comparison now covers only the rows strictly between the two LOWEST rules. Rules are recognised by box-drawing characters alone; ASCII dashes are excluded, since a line of them in the conversation would be taken for the box. A screen with no box found falls back to comparing everything, never to «no box, so no draft» — an unparsed screen must not become permission to wipe. Second half of the same defect: the plaque said «жду тишины · 695 с» while the watcher was serving a refusal, and the human read it as waiting for a pause that had already lasted eleven minutes; `PHASE_COOLING` now names it and counts down. `tests/autoloop_tests/` gains 10 cases built from the measured screens.

- **The window was wiped wherever a 45-second pause happened to fall, and the run was never told the context was filling up (`wrap-up-before-window-wipe`, `winddown-asked-twice-through-the-wrong-channel`).** On reaching the threshold the watcher went straight to `/checkpoint` -> `/clear` -> `/start`, so the cleanup could land halfway through a task; meanwhile nothing informed the agent, which kept taking on more work. The Stop hook `exit_guard` could already ask for a wind-down, but `decide` gated the soft branch on `task_state == STATE_COMPLETE` -- for a task ALREADY closed. Mid-task the agent heard nothing until `hard_threshold` = 75%. That gate is now lifted for the chat branch only: a headless iteration dies after its task regardless, so cutting one off mid-task buys nothing, while a declared run inside a live chat has no such boundary. The chat instruction was rewritten to stop claiming the task is finished and to name the closing steps, since it now arrives mid-work. The watcher keeps the other half: it wipes only once the run is STANDING, asked of the Stop hook's flag rather than of the transcript -- a long tool call writes nothing for minutes and reads exactly like an empty room. Absent flag and absent question are told apart by a sentinel and neither may veto a cleanup, or a project without `chat_ready.py` would never have its window cleaned again.

  Dead end, built and removed inside one session: the wind-down request was first delivered by the watcher TYPING it into the console, with a state machine to wait the run out, a five-second tolerance to recognise its own message coming back as a turn from the human, and a deferred tick so the wipe could not land in the same breath as the request. All of it was the wrong channel. The watcher arms on 45 seconds of transcript silence, which continuous work never produces -- so in the one case the request exists for, it would not have been sent, and in the rest it duplicated the hook, including a second `/checkpoint`. The hook fires at the end of every turn and needs no keystrokes. The warning was already written in `exit_guard`'s own comment and was read after the second mechanism had been built. `autoloop_chat_cycle.py` 576 -> 493 lines.

  Found while restructuring: `run_sequence` left the cycle ARMED and the only thing moving it on was a false cancel firing a tick later against the run's own writing -- visible in the log two seconds after both completed cleanups, at 22:43:24 and 19:09:26. Without that accident the sequence would have repeated every tick; `Maintenance.finish` now does it explicitly. `DEFAULT_SOFT_THRESHOLD` moves 30 -> 50.

- **A here-document body was read as a command line, so a commit was refused over the text of its own message (`push-gate-refuses-a-commit-over-its-message`).** Both gates rest on token boundaries: a real command's words arrive as SEPARATE tokens, a mention inside a quoted argument arrives as ONE. `shlex` does not know the here-document construct, so every word of the body arrived as its own token and prose became command-shaped. Measured: `scan_target` returned `git commit -F - << EOF Дальше git push -f в форк. EOF` -- newlines gone, body welded to the command -- and `tokenize` returned `[... '<<', 'EOF', 'git', 'push', ...]`, which is exactly the shape `_tokens_invoke_git_push` looks for. So `bash_firewall` blocked an ordinary commit as a force-push, and `git_push_gate` refused one as an unticketed push; the workaround (`git commit -F <file>`) had been recorded as a shared gotcha with no defect underneath it. Demonstrated live mid-fix: the firewall blocked the reproduction command itself, having found `git push` and `-F` in the text of a Python script. The body is stdin, not a command line, so it is split off BEFORE tokenizing -- it is tokenizing that turns its prose into words. The exception is an interpreter: `bash <<EOF ... EOF` really does execute its body, so there it is re-scanned as the command line it is, the same treatment a `-c` payload already gets. An unterminated document is returned untouched and judged whole, the direction the rest of the module takes on input it cannot parse. One helper serves both entry points into the POSIX dialect (`bash_cmd_scan.scan_target`, `bash_write_parse.tokenize`), because fixing one and not the other is how the two gates came to disagree. The PowerShell dialect was checked and needed nothing: it keeps a here-string body as one token already. `tests/test_hooks.py` gains 16 cases across three classes.

- **A stalled run waited out twenty minutes of the night it was declared for, and was then told only «carry on» (`anchor-gap-from-config`).** `ANCHOR_SECONDS` was wired into `autoloop_watch` as 1200 with no way to change it per project; it now comes from `.tausik/config.json` → `autoloop.anchor_seconds`, defaulting to 600, with nonsense values (zero, negative, a string) falling back rather than turning the anchor into a delivery every tick. Every guard in `anchor_due` is unchanged and matters more at the shorter interval, so each is pinned separately: work in flight, an armed cleanup, a turn still running, three dead deliveries, and the interval itself. The text was the other half. The anchor reused `continue_step`, which is written for a chat that has just been WIPED — nothing is pending there and the work simply restarts. A run standing for ten minutes is usually standing on a question it asked, and «Продолжай прогон» leaves it free to ask the same question again; nobody is there to answer either one, which is what a declared run means. `anchor_step` tells it to decide for itself, record the decision and the reason through `task log`, and — for something genuinely impassable — to block the task with a reason rather than invent a way around it silently.

- **A tooling heartbeat read as the agent's work, and the context cleanup never came anywhere between 30% and 75% of the window (`heartbeat-blocks-context-cleanup`).** The previous fix (`autoloop-presence-idle-not-work`) split "the agent STARTED this" from "this is actually working", which settled the lazily started language server. A periodic worker escapes both by construction: measured in `D:/asynchronus` on 2026-08-17, the graph server respawns `codebase-memory-mcp cli --index-worker` every 54s, it lives 13s and burns real CPU, so it is fresh every time (age 0) and genuinely busy. That leaves 41s of silence between bursts against the 45s required, and `busy_since` is reset on every busy tick and caps the silence clock — the ceiling is exactly one gap between wake-ups. So the answer is never, not slow: `chat-watch.log` shows the whole evening as "фоновая работа: 1 процессов" → "кончилась" → again, the window sitting at 51–55%, with the busy override starting only at `hard_threshold` = 75%. What the human sees is not a blocked cleanup but faded rules: the `/checkpoint` → `/clear` → `/start` cycle is the only thing that re-reads CLAUDE.md and memory, and without it a long session forgets by the end that tasks must be tracked. The criterion that separates a heartbeat: work the agent started is born BEFORE the transcript stops moving, because the turn keeps writing after the spawn — the legitimate gap is about a second, a heartbeat wakes tens of seconds later, and `TURN_GRACE_SECONDS` = 15 sits between the two. Parentage only INHERITS legitimacy and can never take it away: `gradlew build` started in the turn spawns its daemon deep into the silence, and the daemon stays work through its parent. Selecting by parentage remains rejected — the negative test about `mcp__windows-mcp__PowerShell` is intact and parentage does not decide. An unreadable transcript switches the filter off entirely: a birth time proves nothing without the end of the turn, and pruning on a guess is worse than not pruning. The silence cap moved out of the loop body into `watch.quiet_after_work` so the cadence sensor exercises it rather than its own copy. The plaque and the log now name the process being waited on — "1 проц." sent the human hunting for an agent that did not exist, twice in one run. `tests/autoloop_tests/` gained 12 cases, including five minutes of the measured cadence driven from both sides: on the old selection no cleanup ever happens, on the new one it happens at second 46.
...
- **The chat watcher hands a stalled run its direction back without waiting for the window to fill (`heartbeat-blocks-context-cleanup`, hardened in `anchor-fires-during-a-long-turn`).** The cleanup re-reads the rules as a side effect of `/clear` → `/start`, but it fires on the window, and a run stalls for a different reason — a step ended, nobody started the next one, and the human who would nudge it is away: that is why the run was declared. Twenty minutes after the TURN ENDED (`ANCHOR_SECONDS`) the watcher delivers the same continuation line the cleanup ends with. The end of the turn comes from the host — the mtime of the flag the Stop hook `chat_ready.py` writes — compared against the transcript's last write: flag after write means standing, write after flag means a turn is running. The first version asked the transcript instead ("45s of quiet") and walked into the trap the neighbouring function in that same file warns about: a quiet transcript means the TURN ended, not the work. Measured on a live run at 19:15:32 — the agent was twenty minutes into `gameprobe-run.sh`, the transcript had not moved for any of it, and the anchor delivered; it cost nothing only because the host queued the line instead of interrupting the turn. The busy check did not save it: the command was waiting on a game and burned no CPU. The remaining conditions are ways of not talking over somebody: background work in flight and an armed cleanup (which does strictly more) both stop the nudge, and `last_at` acts as a floor in case a delivery fails, or the next tick would retry two seconds later. Three deliveries in a row that moved no text mean finished, not stalled (`ANCHOR_TRIES`): the watcher logs that and leaves the chat alone for the rest of the run, because typing into an empty queue until morning is worse than stopping and saying so. The price is named rather than hidden: the delivery uses the continuation path, so no `Esc` (a rule derived by measurement — `Esc` before that step interrupts a turn still drawing itself), and a draft left in the chat for 20 minutes will be glued onto the line. 11 cases in `test_autoloop_watch.py`, including that running turn and its converse — hardening the test must not switch the anchor off.
