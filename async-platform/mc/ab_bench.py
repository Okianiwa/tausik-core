...
def purge_cmds() -> list[str]:
    """Чистка ОТРИЦАНИЕМ: список (zombie/item/orb) пропустил 744 чужие сущности,
    ваниль дала 126.3 вместо 90.5 — 28% брака при дрейфе 0.0%. Отчёт §1."""
...
# Шум СРЕДЫ, не отказ ядра: таймаут HTTP к api.minecraftservices.com даёт три
# строки в лог и бракует точку. Поток не Server thread, на старте, на тик не
# влияет. Список НАМЕРЕННО узкий — не «сетевые ошибки вообще». §6 отчёта.
ERROR_IGNORE = (
    "Yggdrasil",
