...
2. **Классификатор риска** оценивает *явную* публикацию: проектно-специфичные маркеры дают **высокий** риск, и запись блокируется, пока после ручной проверки не выставлен `confirm_high_risk` (`scripts/brain_publish_flow.py`). С 1.8 он больше НЕ решает, куда запись попадёт — см. врезку ниже.
...
### С 1.8: хранилища три, и выбираешь ты — само ничего не маршрутизируется

Страница, которую ты читаешь, описывает **Notion**-brain. С 1.8 он больше не
единственное кросс-проектное хранилище, и попасть в него автоматически нельзя.

| Назначение | Где | Как туда попасть |
|---|---|---|
| Этот проект | `.tausik/tausik.db` | по умолчанию — печатать нечего |
| Общее локальное хранилище | `~/.tausik-knowledge/knowledge.db` (переопределяется `$TAUSIK_HOME`) | `--global` у `decide` / `memory add` |
| Notion-brain | твой Notion workspace | `tausik brain move --to-brain`, поимённо, и только так |

Изменились две вещи, обе ломающие:

- **Запись решения больше никуда его не публикует** (решение #221). Раньше
  назначение выбирал классификатор, ища в тексте проектно-специфичные маркеры.
  Он ошибался в ту сторону, которая стоит дорого: шесть внутренних решений
  этого проекта уехали в вики владельца, каждое с пометкой «проектных маркеров
  не обнаружено» — потому что хорошо написанное решение обычно читается
  обобщённо. Видимость — суждение о намерении, и слова его не несут.
- **Общее локальное хранилище — не `~/.tausik/`** (решение #222). Это имя было
  дефектом: поиск проекта идёт вверх и ищет ровно `.tausik`, поэтому хранилище
  в домашней папке захватывало обнаружение для всего, что лежит под ней.
  Теперь оно живёт в `~/.tausik-knowledge/`.

Общее локальное хранилище **не** вычищается — прочти
[whats-new-1.8](whats-new-1.8.md), прежде чем что-то в него класть.
...
2. **Scrubbing linter** (`scripts/brain_scrubbing.py`, поставляется) перехватывает каждую запись до клиента. Отклоняет: абсолютные Windows/POSIX пути, internal-domain URLs, любой текст, совпавший с regex-списком `brain.private_url_patterns`, kebab-слаги похожие на internal-идентификаторы.
3. **Классификатор риска** (`scripts/brain_classifier.py`, поставляется) оценивает *явную* публикацию, а не выбирает назначение. Проектно-специфичные маркеры дают **высокий** риск, и запись отклоняется, пока после ручной проверки не выставлен `confirm_high_risk`. Консервативный default: неоднозначное → высокий риск. С 1.8 он больше ничего не маршрутизирует сам (решение #221).
