...
**105 инструментов** для ИИ-агентов (98 project + 7 brain; v1.4 актуальный счёт, проверено `len(TOOLS)` обоих серверов). MCP-surface зеркалит CLI 1:1 без CLI-only пробелов. Предпочитайте MCP-инструменты shell-вызовам — они атомарны, возвращают структурированные данные и держат контекст чище.
...
| `tausik_epic_done` | Завершить эпик. Отказ, если внутри живые стори/задачи (перечисляются поимённо); `force` продавливает, roadmap помечает несогласованность | `slug`, `force` |
| `tausik_epic_reopen` | Открыть завершённый эпик обратно (→ active), без delete | `slug` |
...
| `tausik_story_done` | Завершить стори. Отказ, если внутри живые задачи (перечисляются поимённо); `force` продавливает, roadmap помечает несогласованность | `slug`, `force` |
| `tausik_story_reopen` | Открыть завершённую стори обратно (→ active), без delete | `slug` |
| `tausik_story_delete` | Удалить (cascade: задачи) | `slug` |
| `tausik_roadmap` | Дерево: epic → story → task. Эпик/стори, помеченный done при живых детях, показывается ВСЕГДА и помечается ⚠, даже без `include_done` | `include_done` |
