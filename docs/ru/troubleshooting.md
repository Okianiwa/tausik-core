...
| `git push` blocked | Защитный gate сработал — это нормально | Авторизуй **двумя отдельными вызовами**: сначала `tausik push-ok`, следующим вызовом `git push`. Связывать их через `&&` бесполезно — хук PreToolUse судит команду до запуска шелла, поэтому `push-ok` из той же строки тикет ещё не написал. Или используй `/ship` / `/commit` — они делают ровно это после твоего "y". Тикет одноразовый, живёт 60 с, привязан к SHA HEAD. Старый `TAUSIK_ALLOW_PUSH=1` удалён в v1.4 (был broken-by-design). Debug bypass: `TAUSIK_SKIP_PUSH_HOOK=1`. |
| Отказ push-гейта называет каталог, куда `push-ok` не клал | Не рассинхрон путей — так выглядит отсутствие тикета | `push-ok` пишет тикет в git-каталог репозитория, а `.tausik/` — лишь запасной путь для checkout'а без git. Хук перебирает обоих кандидатов и берёт первого, **у кого файл существует**; когда файла нет ни у кого, в отказе печатается первый кандидат — как подсказка «положи сюда», а не «искал только здесь». Прежде чем править `cli_push_ok.py` или `_ticket_path`, сделай замер: `_ticket_path(target_dir)` + `.exists()`. Дважды на этом теряли сессию и один раз правили общий хаб под несуществующий дефект. |
| Первый `git push` прошёл гейт и упал сам, повтор говорит «no push ticket» | Тикет одноразовый — его сжёг первый заход | Успешная проверка **удаляет** файл тикета, поэтому повтор без нового `push-ok` всегда упрётся в «no push ticket» — и настоящая причина (сеть, доступ, upstream) теряется из виду. Выдай тикет заново и читай stderr самого `git push`, а не отказ гейта. |
