...
| `git_push_gate.py` | Перед git push из любого инструмента оболочки (Bash, PowerShell, `mcp__windows-mcp__PowerShell`) | Блокирует push без свежего, одноразового тикета, привязанного к SHA HEAD и к самому репозиторию. Тикет живёт в git-каталоге репозитория, куда идёт push (`.tausik/.push_ticket.json` — запасной путь для проекта без git), поэтому авторизовать можно и соседний репозиторий, открытый не в каталоге сессии. `/ship` и `/commit` запускают `tausik push-ok && git push` после вашего "y" — `push-ok` пишет 60-секундный тикет, хук съедает его на следующем push. |
