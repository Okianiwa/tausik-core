...
Цена флага не сводится к слепым зонам, как утверждала эта страница раньше. Да, связи с модулями, которых в срезе нет, деградируют до `Any` и не проверяются — но тот же `Any` ещё и **отбивает коммиты, которые отбивать не за что**: однострочный делегат (`return self.be.task_get(slug)`) возвращает `Any`, как только сосед оказался вне среза, и `no-any-return` блокирует код, который полное дерево принимает. Замер по `scripts/`, каждый модуль развёрнут в одиночку ровно так, как это делает хук: падают 23 модуля из 150, 64 точки — и все 64 относятся к одному этому коду ошибки, других нет. Поэтому гейт гасит `no-any-return` только на `commit` (`trigger_args` в `default_gates.py`) и оставляет его на `task-done`, где та же команда идёт по рабочему дереву, граф полон и вердикт что-то значит. Тот же прогон делает CI.
