...
| `git_push_gate.py` | Before git push from any shell tool (Bash, PowerShell, `mcp__windows-mcp__PowerShell`) | Blocks unless the ticket is fresh, single-use, and bound to both HEAD SHA and the repository being pushed. It lives in that repository's git dir (`.tausik/.push_ticket.json` is the fallback for a project without git), so a repository beside the session directory can be authorized too. `/ship` and `/commit` run `tausik push-ok && git push` after your "y" — `push-ok` writes the 60-second ticket; the hook consumes it on the next push. |
