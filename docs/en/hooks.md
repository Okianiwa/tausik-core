...
That flag costs more than blind spots, and this page used to claim otherwise. Yes, references into modules absent from the slice degrade to `Any` and go unchecked — but the same `Any` also **blocks commits it should not**: a one-line delegate (`return self.be.task_get(slug)`) returns `Any` the moment its neighbour is outside the slice, and `no-any-return` rejects code the full tree accepts. Measured over `scripts/`, each module materialized alone exactly as the hook does it: 23 of 150 modules fail, 64 sites — and all 64 are that one error code, no other. So the gate waives `no-any-return` on `commit` only (`trigger_args` in `default_gates.py`) and keeps it on `task-done`, where the same command runs against the real worktree, the graph is complete and the verdict means something. CI runs the same check.
