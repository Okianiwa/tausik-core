...
- `git_push_gate.py` requires a fresh, single-use ticket written by `tausik push-ok` (60s TTL, bound to HEAD SHA and to the repository it was issued for) in that repository's git dir, falling back to `.tausik/.push_ticket.json`. A ticket taken for one repository does not authorize a push from another. `/ship` and `/commit` run `tausik push-ok && git push` after user "y". The historical `TAUSIK_ALLOW_PUSH=1` env path was broken-by-design (inline env never reached harness-level hooks) and was removed in v1.4. `TAUSIK_SKIP_PUSH_HOOK=1` remains as a debug-only bypass.
