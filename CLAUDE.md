...
Tasks: 89/107 done, 0 active, 0 blocked

### Memory tail
Decisions (5):
- #70 Майн-наработки сведены в репозиторий проекта: код стенда, рабочая память и решения переезжают в базу владельца кода, а з
- #69 Дублирующая доставка артефакта удаляется, а не обкладывается continue-on-error: единственным каналом выдачи клиентского 
...
Conventions (5):
- #143 Переносишь задачу в другой проект — перенеси вместе с ней опровергнутые попытки, иначе их воскресят
- #141 Шаг, закрытый из-за обесценивания цели, помечают отказом в журнале — иначе он читается как выполненн
- #139 Вердикт по доставке снимают с метаданных артефакта, а не с цвета галки
